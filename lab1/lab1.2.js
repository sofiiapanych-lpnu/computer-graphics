...
function drawFigures(ctx, segmentLength, centerX, centerY, start, end){
  ...
  if (values.some(isNaN)) {
    ...
    console.log("angle in radians: ", angle, "degrees: ", angle * (180 / Math.PI));
    ...
    ctx.rotate(angle);
    drawSquare(ctx, sideLength, isBorderChecked, isFillingChecked);
    ctx.restore();
    ...
    function rotatePoint(x, y, x0, y0, alpha) {
      ...
      const newY = x0 + dx * cosAlpha - dy * sinAlpha;
      const newX = y0 + dx * sinAlpha + dy * cosAlpha;
    ...
    console.log("Rotated upperLeft:", rotatedPoint_upperLeft.x, rotatedPoint_upperLeft.y);
    ...
    drawCircle(ctx, isBorderChecked, isFillingChecked);
  }
}
...
function drawSquare(ctx, sideLength, isBorderChecked, isFillingChecked){
  if (isSquareSelected()){
    if (isFillingChecked){
...
function drawCircle(ctx, isBorderChecked, isFillingChecked){
  if (!isSquareSelected()){
    if (isFillingChecked){
      ctx.fillStyle = getColor();
      ctx.fill();
